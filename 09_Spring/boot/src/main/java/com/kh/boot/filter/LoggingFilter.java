...
/*
    Interceptor와 AOP는 스프링 내부에서 동작하는 반면, Filter는 스프링 외부에서 Servlet에 요청이 도착 전에 동작한다.

    로그를 띄우려면 외부에서 전달받은 Request값을 스프링으로 보내기 전에 한 번은 읽어와야 하는데,
    서블릿 요청의 전체값은 스트림으로 전달되기 때문에 딱 한 번만 읽어올 수 있다.
    Filter에서 요청을 읽어 사용하고 스프링으로 전달하면 에러가 발생함
    
    1. 클라이언트가 HTTP 요청 보냄
    2. Filter 요청을 가로채서 전처리 실행
    ...
    - Spring MVC 컨트롤러 내부 동작을 알 수 없음
 */

//OncePerRequestFilter
//일반적인 Filter는 여러번 호출될 위험이 있음
//그래서 한 요청당 필터를 한 번만 실행하여 중복을 방지하는 역할이 더해진 Filter
@Slf4j
//@Component
...
/*
    ...
    public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain filterChain) throws IOException, ServletException {
        ...
        ContentCachingResponseWrapper responseWrapper = new ContentCachingResponseWrapper((HttpServletResponse) servletRequest);
