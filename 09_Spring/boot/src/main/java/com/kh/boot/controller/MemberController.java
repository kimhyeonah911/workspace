...
public class MemberController {
    /*
        ...
        클래스 내에서 필요한 객체를 직접 생성하지 않고 spring 컨테이너가 관리하는 객체(Bean에 등록)를 주입받아서 사용할 수 있게 해줌
        필드주입방식/생성자주입방식

        private MemberService memberService = new MemberServiceImpl();
        기존 객체 생성 방식
        객체간의 결합도가 높아짐(소스코드 수정이 일어날 경우 하나하나 전부 바꿔줘야 한다.)
        ...
        필드 주입 방식
        스프링 컨테이너가 객체를 생성 후, @Autowired 붙은 필드에 의존성을 주입해주는 방식
        장점 : 간결하다. 따로 생성자가 setter를 작성하지 않아도 된다.
        단점 : 테스트가 어려움(필드 주입 방식은 객체 생성시 의존성이 주입되지 않고 bean에서 생성 후 주입받는 방식이기 때문에
                            테스트 진행시 임의의 객체를 생성하기가 어렵다.)
              불변성을 보장할 수 없음. 객체 생성시 의존성이 주입되어 고정되지 않기 때문에 클래스 생성 이후에 의존성이 변경될 수 있음.

        생성자 주입 방식
        스프링 컨테이너가 객체를 생성할 때 @Autowired 어노테이션이 붙은 생성자를 통해 필요한 의존성을 주입하는 방식
        장점 : 불변성 보장, 테스트가 편리하다. 순환 참조 방지
     */
    ...
    /*
        ...
        스프링 컨테이너가 해당 메서드를 호출할 때 자동으로 객체 생성해서 매개변수로 주임
     */
    /*
    ...
    /*
        요청처리 후에 데이터를 담아서 응답하는 방법(포워딩 or url 재요청)
        1. 스프링에서 제공하는 Model객체를 이용하는 방법
        포워딩 할 응답뷰로 전달하고자 하는 데이터를 맵형식(k-v)으로 담을 수 있는 영역
    ...
    //2. HttpSession 이용한 값 저장 후 url 재요청
    /*
    @PostMapping("login.me")
    ...
    /*
        ModelAndView 객체를 이용한 방법 -> 데이터를 담고 리턴형식까지 지정할 수 있음
     */
    @PostMapping("login.me")
    public ModelAndView login(@ModelAttribute Member member, ModelAndView mv, HttpSession session){
        //url 재요청을 원할시 return 내용을 redirect: 재요청 url로 해주면 됨
        ...
        //bCryptPasswordEncoder.matches(평문, 암호문) -> 해당 비밀번호를 암호화된 비밀번호화 일치하면 true 아니면 false 반환
    ...
    public String insertMember(Member member, ModelAndView mv, HttpSession session){
        /*
    ...
    public String updateMember(Member member, ModelAndView mv, HttpSession session){
        int result = memberService.updateMember(member);
        if(result > 0){
            session.setAttribute("alertMsg", "성공적으로 수정이 완료되었습니다.");
    ...
    public String deleteMember(Member member, ModelAndView mv, HttpSession session){
        int result = 0;
        Member loginMember = memberService.loginMember(member.getUserId());
        if(!bCryptPasswordEncoder.matches(member.getUserPwd(), loginMember.getUserPwd())){
            session.setAttribute("alertMsg", "비밀번호가 일치하지 않습니다.");
        } else{
            result = memberService.deleteMember(member.getUserId());
        }
        ...
        if(result > 0){
            ...
            mv.addObject("errorMsg", "회원 탈퇴에 실패하였습니다.");
            mv.setViewName("/common/errorPage");
            return "common/errorPage";
        }
    }
