...
/*
    스프링 어플리케이션은 대부분의 경우에서 MVC 패턴에 따라서 Web Layer -> Business Layer -> Data Layer
    Web Layer : RestAPI를 제공하고, Client 중심의 로직 적용
    Business Layer : 내부의 정책에 따른 로직을 개발 -> 서비스의 핵심으로 주로 비즈니스마다 달라지는 코드(개발자가 집중해야하는 부분)
...
/*
        ...
        @AfterThrowing : 대상 메서드가 비정상적으로 반환(예외를 반환)된 후에 Advice(추가기능)이 실행된다.

        @Around : 대상 메서드를 감싸서 메서드 호출 전후에 Advice(추가기능)이 실행된다.
