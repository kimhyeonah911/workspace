...
const UseEffectTest = () => {
    ...
    //1. 의존성 배열이 없을 때
    useEffect(() => {
        console.log("의존성 배열이 없을 때 : 모든 렌더링마다 실행됨.")
    })

    //2. 빈 배열을 의존성으로 설정했을 때
    useEffect(() => {
        console.log("빈 배열을 의존성으로 설정했을 때 : 컴포넌트가 마운트 될 때 한 번만 실행")
    }, [])

    //3. 의존성 배열에 name을 넣었을 때
    useEffect(() => {
        console.log("의존성 배열에 name을 넣었을 때 : name이 변경될 때만 실행")
    }, [name])
