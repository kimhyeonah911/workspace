...
public class BoardDao {
	...
	public ArrayList<Board> selectList(SqlSession sqlSession, PageInfo pi){
		...
		//offset : 몇 개의 게시글을 건너뛰고 조회할 것인지
		//boardLimit : 몇 개의 게시글을 가지고 올지
	...
	public int updateBoardCount(SqlSession sqlSession, int boardNo) {
		return sqlSession.update("BoardMapper.updateBoardCount" , boardNo);
	}
	
	public Board selectBoard(SqlSession sqlSession, int boardNo) {
		Board b = (Board) sqlSession.selectOne("BoardMapper.selectBoard", boardNo);
		return b;
	}
	
	public ArrayList<Reply> selectReply(SqlSession sqlSession, int boardNo){
		ArrayList<Reply> list = (ArrayList)sqlSession.selectList("BoardMapper.selectReply", boardNo);
		return list;
	}
