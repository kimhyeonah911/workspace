 ...
 *              Dao로부터 반환받은 결과에 따라 성공/실패 여부를 판단하여 응답화면 결정 -> view메서드 호출
 */
public class MemberController {
	 ...
	 */
	public void inserMember(String userId, String userPwd, String userName, String gender,
	...
	public void updateMember(String userId, String email, String phone, String address, String hobby) {
		int result = md.updateMember(userId, email, phone, address, hobby);
		
		if(result > 0) {
			new MemberMenu().updateSuccess("수정 성공하였습니다.");
		}else {
			new MemberMenu().updateFail("수정 실패하였습니다.");
		}
	}
	/**
	...
	public void deleteMember(String userId) {
		...
		if(result > 0) {
			new MemberMenu().deleteSuccess("삭제 성공하였습니다.");
		}else {
			new MemberMenu().deleteFail("삭제 실패하였습니다.");
		}
	}
