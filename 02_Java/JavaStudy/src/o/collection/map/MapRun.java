...
public class MapRun {
	public static void main(String[] args) {
		...
		//저장되는 순서 유지가 안 됨 -> value값이 중복되어도 키값이 중복되지 않으면 저장됨
		...
		//전체 객체에 접근하는 방법
		//Iterator 반복자 이용x for each 이용x 
		//Map의 key는 Set과 매우 유사하다.
		//1. key를 모아서 Set 자료구조의 형태로 반환받는다.
		...
		//entrySet() : Map 인터페이스에서 제공하는 메서드, 맵에 저장된 모든 키-값 쌍을 set형태로 변환한 것
		Set entrySet = hm.entrySet();
		System.out.println(entrySet);
		for(Object key : entrySet) {
			System.out.println(((Entry)key).getKey() + " : " + ((Entry)key).getValue());
		}
	}
