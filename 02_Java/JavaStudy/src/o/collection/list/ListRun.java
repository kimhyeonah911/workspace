...
public class ListRun {
	 ...
	 * 
	 * - 자료구조 : 방대한 데이터를 보다 효율적으로 관리(추가, 삭제, 조회, 정렬, 수정)할 수 있도록 해주는 구조
	 * - 프레임워크 : 이미 만들어져있는 틀(코드더미)
	 */
	public static void main(String[] args) {
		//ArrayList
		List list = new ArrayList(); //크기를 지정해도 되고 안 해도 됨
		...
		//다양한 타입의 데이터를 담을 수 있음 -> 장점2. 여러타입 보관 가능
		System.out.println(list); //list의 특징 : 순서를 유지하면서 담긴다.(0번 인덱스부터 차근차근)
		
		//2. add(int index, E e) : 직접 인덱스를 지정해서 해당 위치에 데이터를 추가할 수 있음
		...
		//4. set(int index, E e) : 해당 인덱스의 값을 전달받은 e 객체로 덮어씌움
		...
		//9. isEmpty() : 컬렉션이 비어있는지 확인하는 메서드 -> boolean
