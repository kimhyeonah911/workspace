...
public class Run {
	 ...
	 * 4. 구현하고자 하는 프로그램 상의 필요한 객체들을 만들기 위해서(생성하기 위해서)
	 *    :클래스라는 설계도가 먼저 필요하다.
	 * 5. 추상화 과정
	 *    1) 내가 구현하고자 하는 프로그램에서 필요한 객체가 뭘까? 생각해봐야한다.
	 *    2) 그 객체들의 공통적인 속성, 기증들을 추출할 것
	 *    3) 추출한 것들을 가지고 내가 구현하고자 하는 프로그램의 "실질적인 목적"에 맞춰 불필요한 속성, 기능 제거
	 *    4) 최종적으로 추려진 속성들을 어떤 자료형, 어떤 변수명으로 사용할 것인지 정의
	 *    
	 *    ex) 학생 관리 프로그램
	 *    1) 학생 관련 객체(김민수훈련생... 최지원훈련생... 김민지훈련생...등등)
	 *    2) 공통적인 속성 및 기능(이름, 나이, 주소, 전화번호, 키, 몸무게, 머리색, 발사이즈, 달리기속도, 먹는다, 달린다, 잔다, 화장실간다, 공부한다, 휴식한다, 게임한다 ... 등등)
	 *    3) 학생 "인적사항"을 관리하는 프로그램 : 이름, 나이, 주소, 전화번호 ...
	 *       학생 "성적"을 관리하는 프로그램 : 이름, 반, 과목별 점수, 성장점수, 성장률 ...
	 ...
	 *        나이를 보관하는 배열 int[] age = {55, 60 ...} -> name과 age 배열 간의 순서를 보장하지 않는다.
	 *    3) 그래서 나온 개념 -> 구조체
	 ...
	 *    ->자바에서는 구조체개념에 좀 더 확장해서 기능(메서드)을 추가한다 = 클래스
	 * */
